...
> 旧版保留参考：语义问题不能语法解决，每操作 Python 子进程成本远大于价值。
...
**当产出涉及复杂任务时**：四个维度按序检查——Completeness（全了吗）→ Consistency（矛盾了吗）→ Groundedness（有证据吗）→ Honesty（粉饰了吗）。顺序不可颠倒：粉饰了再做证据检查是浪费，证据缺失时做一致性检查是空中楼阁。
**不审的后果不是"可能漏"——是"结构性必然漏"**。因为 NL-agent 的输出偏差不是偶发事件，是介质属性。 [Q1加载/Q2扫盲区/Q3查待显化/Q4检假设 — four-quadrant-overview.md]
